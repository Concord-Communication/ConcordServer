...
You may obtain an access token via the [Get Token endpoint](user-endpoints.md#get-token) by supplying your user's username and password. This access token is valid for a limited time, after which you'll need to obtain a new token using your user's credentials again. Additionally, you may also request a new token using your existing token via the [Recycle Token endpoint](user-endpoints.md#recycle-token).
...
If you don't yet have the credentials needed to obtain an access token, you will need to use the [Register New User endpoint](user-endpoints.md#register-new-user) to register your user with a Concord server. The server may accept or reject this request. If it accepts, you will receive your user's id and username as response, which you can then use to obtain a token.
